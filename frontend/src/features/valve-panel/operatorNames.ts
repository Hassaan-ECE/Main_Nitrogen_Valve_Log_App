const OPERATOR_NAMES_STORAGE_KEY = "nitrogenValve.operatorNames";
...
export function addOperatorName(names: string[], name: string) {
  ...
  if (
    !trimmed ||
    normalized.some((operatorName) => operatorNameKey(operatorName) === key)
  ) {
    return normalized;
  }
...
export function matchingOperatorNames(names: string[], query: string) {
  ...
  const startsWith = names.filter((name) =>
    operatorNameKey(name).startsWith(key),
  );
